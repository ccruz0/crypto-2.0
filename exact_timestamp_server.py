#!/usr/bin/env python3
"""
Servidor con timestamp exacto para Crypto.com API
"""
...
def get_exact_timestamp():
    """Obtener timestamp exacto para Crypto.com API"""
    # Usar timestamp actual pero asegurar que sea válido
    current_time = time.time()
    
    # Si el año es 2025, corregir a 2024
    if datetime.fromtimestamp(current_time).year == 2025:
        corrected_time = current_time - (365 * 24 * 60 * 60)  # Restar 1 año
        timestamp = int(corrected_time * 1000)
        logger.info(f"🕐 Timestamp corregido: {timestamp}")
        logger.info(f"🕐 Fecha corregida: {datetime.fromtimestamp(timestamp/1000)}")
    else:
        timestamp = int(current_time * 1000)
        logger.info(f"🕐 Timestamp actual: {timestamp}")
        logger.info(f"🕐 Fecha actual: {datetime.fromtimestamp(timestamp/1000)}")
...
def make_authenticated_request(method, params=None):
    """Realizar petición autenticada con timestamp exacto"""
    try:
        nonce = get_exact_timestamp()
...
def health():
    return jsonify({
        ...
        "message": "Conectándose a TU cartera real con timestamp exacto"
    })

if __name__ == '__main__':
    print("🚀 Iniciando servidor con timestamp exacto...")
    print("📡 Endpoint: http://localhost:8016/api")
    print("✅ Credenciales reales configuradas")
    print("🕐 Timestamp exacto para Crypto.com API")
