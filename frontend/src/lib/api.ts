...
/** Missing-secret recovery (GET /api/settings/secrets-status) */
export interface SecretMissingItem {
  id: string;
  env_var: string;
  blocked_service: string;
  why: string;
  ssm_parameter_name: string | null;
}

export interface AutomationReadinessPayload {
  applicable: boolean;
...
export interface SecretIntakeResponse {
  ...
  env_var: string;
  persisted_to: string[];
  ssm: { status: string; parameter: string | null; detail: string | null };
  container_restart_required: boolean;
  process_env_unchanged_until_restart: boolean;
  message: string;
  recovery?: {
    auto_restart_available: boolean;
    apply_recovery_path: string;
...
export async function submitSecretIntake(
  ...
  envVar: string,
  value: string,
  persistSsm: boolean = true
): Promise<SecretIntakeResponse> {
...
export interface JarvisAuditRunSummary {
  audit_id: string;
  task_id: string | null;
  created_at: string | null;
...
export interface JarvisAuditListResponse {
  audits: JarvisAuditRunSummary[];
}

export interface JarvisAuditRunDetail extends JarvisAuditRunSummary {
  summary: Record<string, unknown>;
  cost_findings: Record<string, unknown>[];
  security_findings: Record<string, unknown>[];
  resource_findings: Record<string, unknown>[];
  recommendations: string[];
}

export async function listJarvisAudits(limit = 20): Promise<JarvisAuditListResponse> {
  const safeLimit = Math.max(1, Math.min(limit, 100));
  return fetchAPI<JarvisAuditListResponse>(`/jarvis/audits?limit=${safeLimit}`);
}

export async function getJarvisAudit(auditId: string): Promise<JarvisAuditRunDetail> {
  return fetchAPI<JarvisAuditRunDetail>(`/jarvis/audits/${encodeURIComponent(auditId)}`);
}

export interface JarvisExecutiveDashboard {
  infrastructure: {
    aws_monthly_spend?: number;
    aws_daily_spend?: number;
    ec2_instances?: number;
    ebs_volumes?: number;
    snapshots?: number;
    elastic_ips?: number;
    last_aws_audit_date?: string | null;
  };
  security: {
    open_findings?: number;
    critical_findings?: number;
    security_groups_exposed_0_0_0_0?: number;
    untagged_resources?: number;
  };
  jarvis_activity: {
    total_tasks_executed?: number;
    total_audits_executed?: number;
    success_rate?: number;
    failed_tasks?: number;
    average_task_cost?: number;
    total_bedrock_cost?: number;
  };
  crypto_health: {
    last_reconciliation_date?: string | null;
    dashboard_portfolio_value?: number;
    exchange_portfolio_value?: number;
    difference_pct?: number;
    reconciliation_status?: string;
  };
  trends: {
    aws_spend?: { date: string | null; monthly?: number; daily?: number }[];
    findings?: { date: string | null; open?: number; critical?: number }[];
    task_volume?: { date: string | null; tasks?: number; audits?: number }[];
  };
  decision_intelligence?: JarvisDecisionIntelligence;
  execution?: {
...
export async function getJarvisExecutiveDashboard(): Promise<JarvisExecutiveDashboard> {
  return fetchAPI<JarvisExecutiveDashboard>('/jarvis/executive');
}

export interface JarvisCryptoAuditRunSummary {
  audit_id: string;
  task_id: string | null;
  created_at: string | null;
  portfolio_difference_usd: number;
  portfolio_difference_pct: number;
  finding_count: number;
  severity: string;
}

export interface JarvisCryptoAuditListResponse {
  audits: JarvisCryptoAuditRunSummary[];
}

export interface JarvisCryptoAuditRunDetail extends JarvisCryptoAuditRunSummary {
  summary: Record<string, unknown>;
  wallet_findings: Record<string, unknown>[];
  position_findings: Record<string, unknown>[];
  valuation_findings: Record<string, unknown>[];
  price_feed_findings: Record<string, unknown>[];
  recommendations: string[];
}

export async function listJarvisCryptoAudits(limit = 20): Promise<JarvisCryptoAuditListResponse> {
  const safeLimit = Math.max(1, Math.min(limit, 100));
  return fetchAPI<JarvisCryptoAuditListResponse>(`/jarvis/crypto-audits?limit=${safeLimit}`);
}

export async function getJarvisCryptoAudit(auditId: string): Promise<JarvisCryptoAuditRunDetail> {
  return fetchAPI<JarvisCryptoAuditRunDetail>(`/jarvis/crypto-audits/${encodeURIComponent(auditId)}`);
}

export type JarvisActionPlanStatus = 'proposed' | 'approved' | 'rejected';
export type JarvisActionPlanSeverity = 'low' | 'medium' | 'high' | 'critical';
export type JarvisActionPlanSourceType = 'aws_audit' | 'crypto_audit' | 'executive_dashboard';

export interface JarvisActionPlanAction {
  title: string;
  description: string;
  impact: string;
  risk: string;
...
export interface JarvisActionPlanSummary {
  plan_id: string;
  source_type: string | null;
  source_id: string | null;
  severity: JarvisActionPlanSeverity;
  estimated_savings_usd: number;
  status: JarvisActionPlanStatus;
  created_at: string | null;
}

export interface JarvisActionPlanListResponse {
  plans: JarvisActionPlanSummary[];
}

export interface JarvisActionPlanDetail extends JarvisActionPlanSummary {
  estimated_risk_reduction: string;
  actions: JarvisActionPlanAction[];
  action_count: number;
  read_only?: boolean;
  execution_performed?: boolean;
}

export async function listJarvisActionPlans(limit = 20): Promise<JarvisActionPlanListResponse> {
  const safeLimit = Math.max(1, Math.min(limit, 100));
  return fetchAPI<JarvisActionPlanListResponse>(`/jarvis/action-plans?limit=${safeLimit}`);
}

export async function getJarvisActionPlan(planId: string): Promise<JarvisActionPlanDetail> {
  return fetchAPI<JarvisActionPlanDetail>(`/jarvis/action-plans/${encodeURIComponent(planId)}`);
}

export async function generateJarvisActionPlan(body: {
  source_type: JarvisActionPlanSourceType;
  source_id: string;
}): Promise<JarvisActionPlanDetail> {
  return fetchAPI<JarvisActionPlanDetail>('/jarvis/action-plans/generate', {
    method: 'POST',
    body: JSON.stringify(body),
  });
}

export interface JarvisExecutivePriorityItem {
  priority?: number;
  title: string;
  reason: string;
...
export interface JarvisExecutiveBlockedItem {
  title: string;
  reason: string;
  blocked_by: string;
}

export interface JarvisExecutiveReportSummary {
  report_id: string;
  generated_at: string | null;
  overall_health_score: number;
  top_priority_count: number;
  quick_win_count: number;
  top_priority_title: string | null;
}

export interface JarvisExecutiveReportListResponse {
  reports: JarvisExecutiveReportSummary[];
}

export interface JarvisExecutionReview {
  active: number;
  blocked: number;
  overdue: number;
  stalled: number;
...
export interface JarvisExecutiveReportDetail {
  report_id: string;
  generated_at: string | null;
  overall_health_score: number;
  top_priorities: JarvisExecutivePriorityItem[];
  quick_wins: JarvisExecutivePriorityItem[];
  strategic_items: JarvisExecutivePriorityItem[];
  blocked_items: JarvisExecutiveBlockedItem[];
  lessons_learned?: string[];
  execution_review?: JarvisExecutionReview;
  ...
  strategic_alignment?: {
    ...
    objectives?: {
      ...
      status: string;
      health: string;
      supporting_initiatives: number;
      is_blocked?: boolean;
      is_overdue?: boolean;
    }[];
    blocked_objectives?: { objective_id: string; title: string; reason: string }[];
    on_track_objectives?: { objective_id: string; title: string; progress_pct: number }[];
    at_risk_objectives?: { objective_id: string; title: string; progress_pct: number }[];
  };
  read_only?: boolean;
  execution_performed?: boolean;
}

export async function listJarvisExecutiveReports(limit = 20): Promise<JarvisExecutiveReportListResponse> {
  const safeLimit = Math.max(1, Math.min(limit, 100));
  return fetchAPI<JarvisExecutiveReportListResponse>(`/jarvis/executive-reports?limit=${safeLimit}`);
}

export async function getJarvisExecutiveReport(reportId: string): Promise<JarvisExecutiveReportDetail> {
  return fetchAPI<JarvisExecutiveReportDetail>(`/jarvis/executive-reports/${encodeURIComponent(reportId)}`);
}

export async function generateJarvisExecutiveReport(): Promise<JarvisExecutiveReportDetail> {
  return fetchAPI<JarvisExecutiveReportDetail>('/jarvis/executive-reports/generate', {
    method: 'POST',
    body: JSON.stringify({}),
  });
}

export type JarvisDecisionType = 'approved' | 'rejected' | 'deferred';
export type JarvisOutcomeType = 'unknown' | 'successful' | 'unsuccessful' | 'partial';

export interface JarvisDecisionSummary {
  decision_id: string;
  created_at: string | null;
  source_type: string | null;
  source_id: string | null;
  plan_id: string | null;
  decision: JarvisDecisionType;
  decision_reason: string;
  outcome: JarvisOutcomeType;
  reviewed_at: string | null;
  reviewed_by: string | null;
}

export interface JarvisDecisionListResponse {
  decisions: JarvisDecisionSummary[];
}

export interface JarvisDecisionDetail extends JarvisDecisionSummary {
  read_only?: boolean;
  execution_performed?: boolean;
}

export interface JarvisDecisionIntelligence {
  ...
  most_common_rejected_recommendation: string | null;
  most_common_rejected_count?: number;
  most_successful_recommendation_type: string | null;
  most_successful_count?: number;
  repeated_findings_count: number;
  lessons_learned?: string[];
  read_only?: boolean;
}

export async function listJarvisDecisions(limit = 50): Promise<JarvisDecisionListResponse> {
  const safeLimit = Math.max(1, Math.min(limit, 200));
  return fetchAPI<JarvisDecisionListResponse>(`/jarvis/decisions?limit=${safeLimit}`);
}

export async function getJarvisDecision(decisionId: string): Promise<JarvisDecisionDetail> {
  return fetchAPI<JarvisDecisionDetail>(`/jarvis/decisions/${encodeURIComponent(decisionId)}`);
}

export async function createJarvisDecision(body: {
  source_type?: string | null;
  source_id?: string | null;
  plan_id?: string | null;
  decision: JarvisDecisionType;
  ...
  outcome?: JarvisOutcomeType;
  reviewed_by?: string | null;
}): Promise<JarvisDecisionDetail> {
  return fetchAPI<JarvisDecisionDetail>('/jarvis/decisions', {
    method: 'POST',
    body: JSON.stringify(body),
  });
}

export async function getJarvisDecisionAnalytics(): Promise<JarvisDecisionIntelligence> {
  return fetchAPI<JarvisDecisionIntelligence>('/jarvis/decision-analytics');
}

export type JarvisInitiativeStatus = 'planned' | 'active' | 'blocked' | 'completed' | 'cancelled';
export type JarvisInitiativeHealth = 'green' | 'yellow' | 'red';
export type JarvisInitiativePriority = 'critical' | 'high' | 'medium' | 'low';

export interface JarvisInitiativeSummary {
  initiative_id: string;
  created_at: string | null;
  updated_at: string | null;
  title: string;
  description: string;
  status: JarvisInitiativeStatus;
  priority: JarvisInitiativePriority;
  owner: string | null;
  target_date: string | null;
  source_type: string | null;
  source_id: string | null;
  progress_pct: number;
  health: JarvisInitiativeHealth;
...
export interface JarvisInitiativeListResponse {
  initiatives: JarvisInitiativeSummary[];
}

export interface JarvisInitiativeDetail extends JarvisInitiativeSummary {
  read_only?: boolean;
  execution_performed?: boolean;
}

export async function listJarvisInitiatives(
  limit = 50,
  status?: JarvisInitiativeStatus,
): Promise<JarvisInitiativeListResponse> {
  const safeLimit = Math.max(1, Math.min(limit, 200));
  const statusParam = status ? `&status=${encodeURIComponent(status)}` : '';
  return fetchAPI<JarvisInitiativeListResponse>(`/jarvis/initiatives?limit=${safeLimit}${statusParam}`);
}

export async function getJarvisInitiative(initiativeId: string): Promise<JarvisInitiativeDetail> {
  return fetchAPI<JarvisInitiativeDetail>(`/jarvis/initiatives/${encodeURIComponent(initiativeId)}`);
}

export async function createJarvisInitiative(body: {
  title: string;
  description?: string;
  status?: JarvisInitiativeStatus;
  priority?: JarvisInitiativePriority;
  ...
  progress_pct?: number;
  blocked_reason?: string | null;
}): Promise<JarvisInitiativeDetail> {
  return fetchAPI<JarvisInitiativeDetail>('/jarvis/initiatives', {
    method: 'POST',
    body: JSON.stringify(body),
  });
}

export async function updateJarvisInitiative(
  initiativeId: string,
  body: {
    title?: string;
    description?: string;
...
export type JarvisFollowupStatus = 'open' | 'acknowledged' | 'resolved' | 'dismissed';
export type JarvisFollowupSeverity = 'low' | 'medium' | 'high' | 'critical';

export interface JarvisFollowupSummary {
  followup_id: string;
  created_at: string | null;
  updated_at: string | null;
  source_type: string;
  ...
  title: string;
  description: string;
  severity: JarvisFollowupSeverity;
  status: JarvisFollowupStatus;
...
export interface JarvisFollowupListResponse {
  followups: JarvisFollowupSummary[];
}

export interface JarvisFollowupDetail extends JarvisFollowupSummary {
  read_only?: boolean;
  execution_performed?: boolean;
}

export interface JarvisFollowupGenerateResponse {
  followups_touched: number;
  followup_ids: string[];
  summary: Record<string, number>;
...
export async function listJarvisFollowups(
  limit = 50,
  status?: JarvisFollowupStatus,
  severity?: JarvisFollowupSeverity,
): Promise<JarvisFollowupListResponse> {
  const safeLimit = Math.max(1, Math.min(limit, 200));
  const params = new URLSearchParams({ limit: String(safeLimit) });
  if (status) params.set('status', status);
  if (severity) params.set('severity', severity);
  return fetchAPI<JarvisFollowupListResponse>(`/jarvis/followups?${params.toString()}`);
}

export async function getJarvisFollowup(followupId: string): Promise<JarvisFollowupDetail> {
  return fetchAPI<JarvisFollowupDetail>(`/jarvis/followups/${encodeURIComponent(followupId)}`);
}

export async function generateJarvisFollowups(sendTelegram = true): Promise<JarvisFollowupGenerateResponse> {
  return fetchAPI<JarvisFollowupGenerateResponse>(
    `/jarvis/followups/generate?send_telegram=${sendTelegram ? 'true' : 'false'}`,
    { method: 'POST' },
  );
}

export async function updateJarvisFollowup(
  followupId: string,
  body: {
    status?: JarvisFollowupStatus;
    severity?: JarvisFollowupSeverity;
