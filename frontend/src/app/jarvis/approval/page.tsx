  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-400">
              Advanced change gates (Phase 5). Prefer Ops → Jarvis for dry-run investigation approvals.
            </p>
          </div>
          <Link href="/?tab=jarvis" className="shrink-0 text-sm text-indigo-300 hover:text-indigo-200">
            ← Ops → Jarvis
          </Link>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl space-y-4 px-6 py-6">
        <div
          data-testid="jarvis-approval-soft-redirect"
          className="rounded-lg border border-indigo-800/60 bg-indigo-950/40 px-4 py-3 text-sm text-indigo-100"
        >
          <p className="font-medium">Looking to approve a dry-run investigation?</p>
          <p className="mt-1 text-indigo-200/90">
            Use{' '}
            <Link href="/?tab=jarvis" className="underline hover:text-white">
              Ops → Jarvis
            </Link>{' '}
            and tap <span className="font-medium">Approve investigation</span> (continues the plan only).
            ...
            {selected && (
              <div className="space-y-4 text-sm">
                ...
                <div>
                  ...
                  <p data-testid="jarvis-approval-status">{gateLabel(selected.status)}</p>
                </div>
                ...
                <div className="space-y-2 border-t border-slate-800 pt-3">
                  <p className="text-xs uppercase text-slate-500">Primary action</p>
                  <button
                    type="button"
                    data-testid="jarvis-approval-reject"
                  ...
                  <div className="mt-3 space-y-3">
                    ...
                    {writeGatesOff && (
                      <p className="text-xs text-slate-500">
                        Write flags are off (safe default). These buttons stay disabled until flags are
                        explicitly enabled — that is intentional.
                      </p>
                    )}
                ...
                <p className="text-xs text-slate-600 border-t border-slate-800 pt-3">
                  ...
                  Merge and deploy are permanently disabled. Send to LAB / Promote are not on this screen yet.
                </p>
              </div>
            )}
          </section>
