...
import { JarvisAuditRunSummary, listJarvisAudits } from '@/lib/api';
...
export default function JarvisAuditsPage() {
  const [audits, setAudits] = useState<JarvisAuditRunSummary[]>([]);
  ...
  const loadAudits = useCallback(async () => {
    setLoading(true);
    try {
      const data = await listJarvisAudits(20);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900">
      <div className="container mx-auto p-4 md:p-8 max-w-7xl">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">AWS Infrastructure Audits</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Read-only audit history — no resources modified.
            </p>
          </div>
        ...
        {loading && audits.length === 0 ? (
          ...
          <div className="text-center py-8 text-gray-500 border rounded-lg bg-white dark:bg-slate-800">
            No audit runs yet. Run &quot;Run AWS infrastructure audit&quot; from{' '}
          ...
          <Table className="dark:bg-slate-800 dark:border-slate-700">
            <thead className="bg-gray-50 dark:bg-slate-900 text-left text-xs uppercase text-gray-500 dark:text-gray-400">
              <tr>
                ...
                <th className="px-4 py-3 text-center">Resource</th>
              ...
              {audits.map((row) => (
                <tr key={row.audit_id} className="hover:bg-gray-50 dark:hover:bg-slate-800/60">
                  ...
                  <td className="px-4 py-3 text-right text-sm tabular-nums font-mono">
                    ${row.estimated_monthly_savings.toFixed(2)}
                  </td>
                  <td className="px-4 py-3 text-center text-sm">{row.finding_counts?.cost ?? 0}</td>
                  <td className="px-4 py-3 text-center text-sm">{row.finding_counts?.security ?? 0}</td>
                  <td className="px-4 py-3 text-center text-sm">{row.finding_counts?.resource ?? 0}</td>
                  <td className="px-4 py-3">
                    <Badge variant={severityVariant(row.severity)}>{row.severity}</Badge>
                  </td>
                  <td className="px-4 py-3">
                    <Link
                      href={`/jarvis/audits/${row.audit_id}`}
