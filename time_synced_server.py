#!/usr/bin/env python3
"""
Servidor con sincronización de tiempo corregida para Crypto.com API
"""
...
def get_correct_timestamp():
    """Obtener timestamp correcto (2024, no 2025)"""
    # Forzar timestamp de 2024 para evitar problemas de sincronización
    current_time = time.time()
    # Si el año es 2025, ajustar a 2024
    if datetime.fromtimestamp(current_time).year == 2025:
        # Restar un año en segundos
        corrected_time = current_time - (365 * 24 * 60 * 60)
        logger.info(f"🕐 Timestamp corregido: {datetime.fromtimestamp(corrected_time)}")
...
def make_authenticated_request(method, params=None):
    """Realizar petición autenticada con timestamp corregido"""
    try:
        ...
        logger.info(f"🕐 Timestamp usado: {nonce} ({datetime.fromtimestamp(nonce/1000)})")
...
def health():
    return jsonify({
        ...
        "message": "Conectándose a TU cartera real con timestamp corregido"
    })

if __name__ == '__main__':
    print("🚀 Iniciando servidor con sincronización de tiempo corregida...")
    print("📡 Endpoint: http://localhost:8006/api")
    print("✅ Credenciales reales configuradas")
    print("🕐 Timestamp corregido para evitar problemas de sincronización")
