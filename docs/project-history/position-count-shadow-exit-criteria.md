...
| 28-ago | **APT** | 0 | 3 | **ninguno**: verdad = 1 corto real (173.10 ≈ wallet −173.49); legacy dice 0, sombra dice 3 | **CAUSA RAÍZ ENCONTRADA (28-ago tarde)**, cadena de tres eslabones: (1) dos cortos pequeños del 2-ago (17.65, `…6823562`) y 3-ago (17.54, `…6389211`) cuyas protecciones TP/SL **nunca ejecutaron** y fueron **canceladas en masa el 11-ago 05:35** (limpieza de fantasmas); (2) sus cierres reales no existen o no están vinculados en `exchange_orders`, así que el FIFO los mantiene abiertos para siempre — son **ghost lots**; (3) la regla del alineador que clava lots desnudos alineados con la dirección del wallet (`pinable_naked`, añadida por el caso ETH_USDT `…1134850`) los clava **uno a uno sin comprobar que la SUMA clavada quepa en \|wallet\|**: 173.10 protegido + 17.65 + 17.54 = 208.29 > 173.49 → `lots_exceed_wallet` y count=3. El alineador SÍ descartó otros dos fantasmas (10.48 del 7-ago y 2.29 largo) — estos dos sobreviven precisamente por ser pequeños y del mismo signo que el wallet. **ARREGLADO (28-ago tarde)**: el tope va en el CONTADOR (`_cap_lots_to_wallet_for_count`, `position_count_shadow.py`), no en el alineador — el display de Expected TP conserva su semántica deliberada. Verificado contra producción: APT count 3→1 con `ghosts_dropped=2`, resto de símbolos idénticos con cuadre a 8 decimales. La línea de la sombra emite ahora `ghosts_dropped=N`. Deuda restante: los ghost lots siguen en los libros (limpieza aparte) |
