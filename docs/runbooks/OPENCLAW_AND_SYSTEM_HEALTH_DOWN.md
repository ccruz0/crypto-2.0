...
| 504 on `/openclaw/` | **Automated (no SSH):** From your machine run `./scripts/openclaw/fix_504_via_eice.sh` (uses EC2 Instance Connect). Or **GitHub Actions → Fix OpenClaw 504 (EICE)** (manual or runs 06:00/18:00 UTC). **On PROD:** `cd /home/ubuntu/automated-trading-platform && sudo bash scripts/openclaw/fix_504_on_prod.sh`. Then test https://dashboard.hilovivo.com/openclaw/ (expect 401). If still 504, see OPENCLAW_504_UPSTREAM_DIAGNOSIS and ensure LAB is running OpenClaw and SG allows PROD→LAB:8081. |
