...
- **Check SignalMonitor logs:**
  ```bash
  cd /Users/carloscruz/crypto-2.0 && bash scripts/aws_backend_logs.sh --tail 500 | grep -E 'DEBUG_STRATEGY_FINAL|DEBUG_BUY_FLAGS|DEBUG_RESOLVED_PROFILE'
  ```
...
### Backend Logs
```bash
cd /Users/carloscruz/crypto-2.0 && bash scripts/aws_backend_logs.sh --tail 500 | grep DEBUG_STRATEGY_FINAL
```
