...
1. **Check logs** for `[RISK_THROTTLED]` entries:
   ```bash
   cd /Users/carloscruz/crypto-2.0 && bash scripts/aws_backend_logs.sh | grep RISK_THROTTLED
   ```
...
3. **Check risk check logs**:
   ```bash
   cd /Users/carloscruz/crypto-2.0 && bash scripts/aws_backend_logs.sh | grep RISK_PORTFOLIO_CHECK
   ```
