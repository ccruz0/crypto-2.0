#!/usr/bin/env python3
"""
Diagnóstico completo: ¿Por qué no salta la alerta de compra para BTC?
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            if response.status_code == 404:
                print(f"   → {SYMBOL} no está en la watchlist")
            return
        ...
        print("-" * 70)
        print(f"   • Precio: ${price:,.2f}" if price else "   • Precio: N/A")
        print(f"   • RSI: {rsi:.2f}" if rsi is not None else "   • RSI: N/A")
        print(f"   • EMA10: ${ema10:,.2f}" if ema10 else "   • EMA10: N/A")
        print(f"   • MA50: ${ma50:,.2f}" if ma50 else "   • MA50: N/A")
        print(f"   • MA200: ${ma200:,.2f}" if ma200 else "   • MA200: N/A")
        print(f"   • Volume Ratio: {volume_ratio:.2f}x" if volume_ratio is not None else "   • Volume Ratio: N/A")
        print()
        ...
        if response.status_code == 200:
            ...
            if "reasons" in signals_data:
                reasons = signals_data.get("reasons", [])
                if reasons:
            ...
            if not buy_signal:
                ...
                if ema10 and price:
                    ...
                    if price <= ema10:
                        issues.append(f"   ⚠️  Precio ${price:,.2f} <= EMA10 ${ema10:,.2f} (diff: {price_diff_pct:.2f}%)")
                    else:
                        print(f"   ✅ Precio ${price:,.2f} > EMA10 ${ema10:,.2f} (diff: {price_diff_pct:.2f}%)")
                
                # MA50 check (si está habilitado)
                if ma50 and price:
                    ma50_diff_pct = ((price - ma50) / ma50) * 100 if ma50 > 0 else 0
                    if price <= ma50:
                        print(f"   ⚠️  Precio ${price:,.2f} <= MA50 ${ma50:,.2f} (diff: {ma50_diff_pct:.2f}%)")
                    else:
                        print(f"   ✅ Precio ${price:,.2f} > MA50 ${ma50:,.2f} (diff: {ma50_diff_pct:.2f}%)")
                ...
                if volume_ratio is not None:
                    ...
                    print("   ⚠️  Volume ratio no disponible")
