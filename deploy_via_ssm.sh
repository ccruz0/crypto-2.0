...
if [ "$MODE" = "fast" ]; then
    ...
    COMMAND_ID=$(aws ssm send-command \
        ...
        --parameters 'commands=["set -e","cd /home/ubuntu/automated-trading-platform 2>/dev/null || cd /home/ubuntu/crypto-2.0 || exit 1","git config --global --add safe.directory /home/ubuntu/automated-trading-platform 2>/dev/null || true","git config --global --add safe.directory /home/ubuntu/crypto-2.0 2>/dev/null || true","git pull origin main || true","docker compose --profile aws ps","docker compose --profile aws restart backend-aws || docker compose --profile aws up -d","sleep 20","curl -sf --connect-timeout 5 http://127.0.0.1:8002/ping_fast && echo Backend OK || echo Backend not ready","sudo systemctl restart nginx 2>/dev/null || true","docker compose --profile aws ps"]' \
        --region $REGION \
        --timeout-seconds 300 \
        --output text \
