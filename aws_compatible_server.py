#!/usr/bin/env python3
"""
Servidor compatible con AWS - usando timestamp de 2024
"""
...
def get_aws_compatible_timestamp():
    """Obtener timestamp compatible con AWS (2024)"""
    current_time = time.time()
    # Si el año es 2025, restar exactamente 1 año para que sea 2024
    if datetime.fromtimestamp(current_time).year == 2025:
        ...
        logger.info(f"🕐 Timestamp corregido para AWS: {timestamp}")
...
def make_authenticated_request(method, params=None):
    """Realizar petición autenticada compatible con AWS"""
    try:
        nonce = get_aws_compatible_timestamp()
...
def health():
    return jsonify({
        ...
        "message": "Conectándose a TU cartera real (compatible con AWS)"
    })

if __name__ == '__main__':
    print("🚀 Iniciando servidor compatible con AWS...")
    print("📡 Endpoint: http://localhost:8014/api")
    print("✅ Credenciales reales configuradas")
    print("🕐 Timestamp compatible con AWS (2024)")
