...
def test_notify_unexpected_block():
    ...
    # Still considered noteworthy for monitoring persistence / daily rollup.
    assert (
...
def test_max_open_orders_total_never_sends_live_telegram():
    from app.utils.trading_guardrails import should_send_trade_block_telegram_alert

    reason = "blocked: MAX_OPEN_ORDERS_TOTAL limit reached (27/10)"
    assert should_send_trade_block_telegram_alert("DOT_USD", "BUY", reason) is False
    assert should_send_trade_block_telegram_alert("ETH_USDT", "SELL", reason) is False
...
def test_trade_block_cooldown_dedupes_repeated_alerts(monkeypatch):
    ...
    reason_a = "blocked: MAX_ORDERS_PER_SYMBOL_PER_DAY limit reached (2/2)"
    reason_b = "blocked: MAX_ORDERS_PER_SYMBOL_PER_DAY limit reached (3/2)"
