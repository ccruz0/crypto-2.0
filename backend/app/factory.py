...
from app.brief import router as brief_router
...
if not is_atp_trading_only():
    ...
    ai_router = None  # type: ignore[assignment, misc]
    agent_router = None  # type: ignore[assignment, misc]
    governance_router = None  # type: ignore[assignment, misc]
    github_webhook_router = None  # type: ignore[assignment, misc]
try:
...
# Use "app.main" so log lines match historical deployments (module now lives in factory.py).
...
def create_app(role: str = "legacy") -> FastAPI:
    """Build the FastAPI application. Only role='legacy' is supported (production / gunicorn entrypoint)."""
    if role != "legacy":
        raise ValueError("create_app only supports role='legacy'.")
    ...
    async def startup_event():
        ...
        # Cursor Execution Bridge — startup visibility (LAB/automation only)
        if not is_atp_trading_only():
            try:
        ...
        # Notion integration: validate env at startup (automation / agent path only)
        if not is_atp_trading_only():
            try:
            ...
            logger.info("Notion startup validation skipped (ATP_TRADING_ONLY=1)")

        # Deploy secrets diagnostics: log presence (never values) for operator visibility.
        # Trading-only: do not import github_app_auth at startup (no deploy/GitHub App path).
        _gh_ws = bool((os.getenv("GITHUB_WEBHOOK_SECRET") or "").strip())
        _oc = bool((os.getenv("OPENCLAW_API_TOKEN") or "").strip())
        if is_atp_trading_only():
            logger.info(
                "ATP_TRADING_ONLY=1 — deploy/agent secrets not required; "
                "GitHub App auth startup check skipped; OPENCLAW_API_TOKEN=%s",
                "present" if _oc else "absent",
            )
        else:
            try:
                from app.services.github_app_auth import (
                    diagnose_github_app_config,
                    github_api_token_configured,
                    is_github_app_configured,
                    legacy_pat_allowed,
                    log_redundant_github_token_if_app_active,
                )

                _gh_ok = github_api_token_configured()
                _gh_app = is_github_app_configured()
                _gh_legacy = bool((os.getenv("GITHUB_TOKEN") or "").strip())
                _legacy_allow = legacy_pat_allowed()
            except Exception as _ga_exc:
                logger.warning("GitHub auth module load failed (treating as misconfigured): %s", _ga_exc)
                _gh_ok = False
                _gh_app = False
                _gh_legacy = bool((os.getenv("GITHUB_TOKEN") or "").strip())
                _legacy_allow = False
            try:
                diag = diagnose_github_app_config()
                logger.info("GitHub auth diagnostics: %s", diag)
            except Exception:
                pass
            logger.info(
                "Deploy secrets: github_auth=%s (app=%s legacy_token_env=%s ALLOW_LEGACY=%s) "
                "GITHUB_WEBHOOK_SECRET=%s OPENCLAW_API_TOKEN=%s",
                "present" if _gh_ok else "MISSING",
                "yes" if _gh_app else "no",
                "yes" if _gh_legacy else "no",
                "yes" if _legacy_allow else "no",
            ...
            try:
                ...
                pass
            # Block legacy GITHUB_TOKEN on AWS unless explicit emergency escape hatch
            if is_aws() and _gh_legacy and not _gh_app:
                if not _legacy_allow:
                    raise RuntimeError(
                        "GITHUB_TOKEN in environment is no longer supported on AWS without GitHub App. "
                        "Configure GitHub App via SSM + scripts/aws/render_runtime_env.sh (see backend/docs/GITHUB_APP_AUTH.md). "
                        "Emergency rollback only: set ALLOW_LEGACY_GITHUB_PAT=true in secrets/runtime.env and recreate backend-aws."
                    )
                logger.warning(
            ...
            if is_aws() and not _gh_ok and _execution_ctx != "LAB":
                raise RuntimeError(
                    ...
                    "scripts/aws/render_runtime_env.sh + SSM. "
                    "Emergency only: ALLOW_LEGACY_GITHUB_PAT=true and GITHUB_TOKEN in secrets/runtime.env. "
                    "Then: docker compose --profile aws up -d --force-recreate backend-aws"
                )
        ...
        async def _background_init():
            ...
            try:
                ...
                _run_agent_scheduler = _run_poller and not is_atp_trading_only()
                if _run_agent_scheduler:
                    try:
                    ...
                    if is_atp_trading_only():
                        logger.info(
                            "Agent scheduler loop skipped (ATP_TRADING_ONLY=1 — trading-only process)"
                        )
                    else:
                        logger.info(
    ...
    async def root():
        ...
        return {
            ...
            "version": "0.40.0",
            "status": "running",
            "atp_trading_only": is_atp_trading_only(),
        }
    ...
    if _admin_router_available and admin_router is not None:
