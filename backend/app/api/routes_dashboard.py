...
def get_dashboard(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if total_time > 1.0:
            log.warning(f"⚠️ Dashboard endpoint took {total_time:.3f}s - this is slow! Should be < 0.5 seconds.")
    ...
    try:
        ...
        try:
            ...
            if "trade_enabled" in item:
                ...
                log.info(f"🔵 Received trade_enabled in request: {new_value} (type: {type(new_value).__name__})")
                watchlist_item.trade_enabled = new_value
                log.info(f"✅ Updated trade_enabled for {watchlist_item.symbol}: {old_value} -> {watchlist_item.trade_enabled}")
            else:
                # trade_enabled not in request - preserve existing value (don't change it)
                log.debug(f"ℹ️ trade_enabled NOT in request data for {watchlist_item.symbol} - preserving existing value: {watchlist_item.trade_enabled}")
            if "trade_amount_usd" in item:
        ...
        try:
            db.commit()
            log.info(f"✅ Database commit successful for item {item_id}")
        except Exception as commit_err:
            log.error(f"❌ Database commit failed for item {item_id}: {commit_err}", exc_info=True)
            try:
                db.rollback()
            except:
                pass
        ...
        try:
            db.refresh(watchlist_item)
            log.info(f"✅ Refreshed watchlist item {item_id} from database")
        except Exception as refresh_err:
            log.warning(f"⚠️ Failed to refresh item {item_id} from database (non-critical): {refresh_err}")
            ...
            try:
                ...
                if fresh_item:
                    ...
                    if final_trade_enabled != expected_value:
                        log.error(f"❌ CRITICAL: trade_enabled mismatch! Expected {expected_value}, got {final_trade_enabled}")
                        ...
                        try:
                            ...
                            log.info(f"✅ Fixed trade_enabled mismatch for item {item_id}")
                        except Exception as fix_err:
                            log.error(f"❌ Failed to fix trade_enabled mismatch: {fix_err}")
                    else:
                        watchlist_item.trade_enabled = final_trade_enabled  # Ensure local reference matches DB
            except Exception as verify_err:
                log.warning(f"⚠️ Could not verify trade_enabled from database: {verify_err}")
        
        log.info(f"✅ Successfully updated watchlist item {item_id} ({watchlist_item.symbol}): trade_enabled={watchlist_item.trade_enabled}, trade_amount_usd={watchlist_item.trade_amount_usd}")
...
def get_dashboard_state(db: Session = Depends(get_db)):
    ...
    """
    # ⚡ Fast-path: devolver datos reales pero SIN operaciones pesadas
    if DEBUG_DASHBOARD_FAST_PATH:
        try:
            ...
            # Convert balances to assets format for frontend
            assets = []
            for bal in balances:
                if bal.get("usd_value", 0) > 0:  # Only include assets with value
                    assets.append({
                        "coin": bal.get("currency", ""),
                        "balance": bal.get("balance", 0),
                        "available_qty": bal.get("balance", 0),  # Frontend expects this
                        "reserved_qty": 0,  # Not tracked in portfolio_balances
                        "value_usd": bal.get("usd_value", 0),  # Frontend expects value_usd
                        "pct_portfolio": (bal.get("usd_value", 0) / total_usd * 100) if total_usd > 0 else 0
                    })
    ...
    # OPTIMIZED: Full dashboard functionality restored with performance optimizations
    ...
    import time as time_module
    start_time = time_module.time()
    MAX_TIME_BUDGET_S = 2.5  # Keep endpoint responsive under load
    ...
    try:
        ...
        # If we're close to the time budget, short-circuit and return balances-only snapshot
        elapsed_after_balances = time_module.time() - start_time
        if elapsed_after_balances > MAX_TIME_BUDGET_S * 0.6:
            log.warning(f"⏱️ Time budget nearly exceeded ({elapsed_after_balances:.2f}s) - returning balances-only snapshot")
        ...
        # OPTIMIZED: Only fetch recent open orders (last 50) to keep response fast
        ...
        # OPTIMIZED: Limit to 50 most recent open orders to prevent reading thousands of rows
        ...
        # FUTURE OPTIMIZATION: Consider creating a materialized view or separate table for recent open orders
        def fetch_open_orders():
            # Import here to avoid scope issues
            from app.models.exchange_order import ExchangeOrder, OrderStatusEnum
            
            # Use explicit filter to ensure we only get open orders
            # Order by exchange_create_time DESC to get most recent first
            # Limit to 50 to prevent reading thousands of rows
            # IMPORTANT: This query should use the index on status and exchange_create_time
            # If this query is still slow, check that indexes exist on:
            #   - exchange_orders.status
            #   - exchange_orders.exchange_create_time
            try:
                # Use explicit filter with status enum values to ensure index usage
                ...
                log.debug(f"Fetched {len(result)} open orders from database (limited to 50)")
                return result
            except Exception as e:
        ...
        log.info(f"✅ Dashboard state fetched in {elapsed_time:.2f}s: {len(balance_data)} balances, {len(fast_signals_data)} fast signals, {len(slow_signals_data)} slow signals, {len(open_orders_data)} open orders")
        
        if elapsed_time > 5.0:
            log.warning(f"⚠️ Dashboard state fetch took {elapsed_time:.2f}s - this is slow! Should be < 1 second")
