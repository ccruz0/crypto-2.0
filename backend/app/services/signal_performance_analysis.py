...
from app.services.agent_execution_policy import ATTR_SAFE_LAB_APPLY
...
def _ensure_analysis_index(path: Path, task_id: str, title: str) -> None:
    path_guard.safe_mkdir_lab(path.parent, context="signal_performance_analysis:index_parent")
    header = (
    ...
    if not path.exists():
        path_guard.safe_write_text(path, header, context="signal_performance_analysis:index_header")
    line = f"- [Signal performance {task_id}: {title}](signal-performance-{task_id}.md)"
    content = path.read_text(encoding="utf-8", errors="ignore")
    if line not in content:
        chunk = ("" if content.endswith("\n") else "\n") + line + "\n"
        path_guard.safe_append_text(path, chunk, context="signal_performance_analysis:index_append")
...
def apply_signal_performance_analysis_task(prepared_task: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        path_guard.safe_mkdir_lab(out_dir, context="signal_performance_analysis:out_dir")
        ...
        path_guard.safe_write_text(out_file, content, context="signal_performance_analysis:analysis_md")
...
setattr(apply_signal_performance_analysis_task, ATTR_SAFE_LAB_APPLY, True)
