...
def merge_orders(
    ...
    def upsert(unified: Optional[UnifiedOpenOrder]) -> None:
        ...
        if not existing:
            merged[unified.order_id] = unified
            return
        # Prefer trigger orders (more metadata)
        if unified.is_trigger and not existing.is_trigger:
            merged[unified.order_id] = unified
            return
        # Otherwise keep entry with more data
        def _richness(order: UnifiedOpenOrder) -> int:
            score = 0
            for attr in ("trigger_price", "price", "trigger_type", "client_oid"):
                if getattr(order, attr):
                    score += 1
            return score
        if _richness(unified) > _richness(existing):
            merged[unified.order_id] = unified
...
def calculate_portfolio_order_metrics(
    ...
    - open_orders_count
    - tp: Highest SELL order above market (or any TAKE_PROFIT if no market)
    - sl: Lowest SELL order below market (or any STOP_LOSS if no market)
    """
    metrics: Dict[str, Dict[str, Optional[float]]] = {}
    market_prices = market_prices or {}
