...
def _ensure_analysis_index(path: Path, task_id: str, title: str) -> None:
    path_guard.safe_mkdir_lab(path.parent, context="agent_strategy_analysis:index_parent")
    header = (
    ...
    if not path.exists():
        path_guard.safe_write_text(path, header, context="agent_strategy_analysis:index_header")
    entry = f"- [Notion task {task_id}: {title}](notion-task-{task_id}.md)"
    content = path.read_text(encoding="utf-8", errors="ignore")
    if entry not in content:
        chunk = ("" if content.endswith("\n") else "\n") + entry + "\n"
        path_guard.safe_append_text(path, chunk, context="agent_strategy_analysis:index_append")
...
def apply_strategy_analysis_task(prepared_task: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        path_guard.safe_mkdir_lab(analysis_dir, context="agent_strategy_analysis:out_dir")
        analysis_file = analysis_dir / f"notion-task-{task_id}.md"
        path_guard.safe_write_text(analysis_file, md, context="agent_strategy_analysis:analysis_md")
...
setattr(apply_strategy_analysis_task, ATTR_SAFE_LAB_APPLY, True)
