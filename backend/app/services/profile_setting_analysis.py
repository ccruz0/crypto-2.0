...
def _ensure_analysis_index(path: Path, task_id: str, title: str) -> None:
    path_guard.safe_mkdir_lab(path.parent, context="profile_setting_analysis:index_parent")
    header = (
    ...
    if not path.exists():
        path_guard.safe_write_text(path, header, context="profile_setting_analysis:index_header")
    entry = f"- [Profile settings {task_id}: {title}](profile-settings-{task_id}.md)"
    content = _read_text(path)
    if entry not in content:
        chunk = ("" if content.endswith("\n") else "\n") + entry + "\n"
        path_guard.safe_append_text(path, chunk, context="profile_setting_analysis:index_append")
...
def apply_profile_setting_analysis_task(prepared_task: dict) -> dict:
    try:
        ...
        path_guard.safe_mkdir_lab(out_dir, context="profile_setting_analysis:out_dir")
        out_file = out_dir / f"profile-settings-{task_id}.md"
        path_guard.safe_write_text(
            out_file,
            "\n".join(lines).strip() + "\n",
            context="profile_setting_analysis:analysis_md",
        )
...
setattr(apply_profile_setting_analysis_task, ATTR_PROD_MUTATION, True)
