...
def should_emit_signal(
    *,
    ...
    """
    Determine if a signal should be emitted based on throttling rules.
    ...
    elapsed_minutes = (now_ts - last_same_time).total_seconds() / 60.0
    min_interval = max(config.min_interval_minutes or 0.0, 0.0)
    cooldown_met = elapsed_minutes >= min_interval

    last_price = last_same_side.price
    if last_price and last_price > 0:
        price_change_pct = abs((current_price - last_price) / last_price * 100)
    else:
        price_change_pct = 999.0  # No reliable reference price -> treat as large change
    min_pct = max(config.min_price_change_pct or 0.0, 0.0)
    price_met = price_change_pct >= min_pct

    if cooldown_met or price_met:
        reason = []
        if cooldown_met:
            reason.append(
                f"cooldown OK ({elapsed_minutes:.2f}m >= {min_interval:.2f}m)"
            )
        if price_met:
            reason.append(
                f"price change OK ({price_change_pct:.2f}% >= {min_pct:.2f}%)"
            )
        return True, "; ".join(reason)

    return False, (
        f"Throttled {symbol} {side}: "
        f"elapsed {elapsed_minutes:.2f}m < {min_interval:.2f}m AND "
        f"price change {price_change_pct:.2f}% < {min_pct:.2f}%"
    )
