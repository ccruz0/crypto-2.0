...
do_deploy() {
  ...
  run1="sudo docker run -d --restart unless-stopped -p 8080:18789 --group-add $DOCKER_GID -v /var/run/docker.sock:/var/run/docker.sock -e OPENCLAW_ALLOWED_ORIGINS=$OPENCLAW_ALLOWED_ORIGINS -e OPENCLAW_TRUSTED_PROXIES=$OPENCLAW_TRUSTED_PROXIES -e OPENCLAW_CONFIG_PATH=$OPENCLAW_CONFIG_IN_CONTAINER$model_env_flags -v $OPENCLAW_HOME_DIR:/home/node/.openclaw -v $ATP_REPO_PATH:/home/node/.openclaw/workspace:ro -v $OPENCLAW_HOME_DIR/agents:/home/node/openclaw/agents --name openclaw $OPENCLAW_IMAGE"
  run2="sudo docker run -d --restart unless-stopped -p 8080:18789 --group-add $DOCKER_GID -v /var/run/docker.sock:/var/run/docker.sock -e OPENCLAW_ALLOWED_ORIGINS=$OPENCLAW_ALLOWED_ORIGINS -e OPENCLAW_TRUSTED_PROXIES=$OPENCLAW_TRUSTED_PROXIES -e OPENCLAW_CONFIG_PATH=$OPENCLAW_CONFIG_IN_CONTAINER$model_env_flags -v $OPENCLAW_HOME_DIR:/home/node/.openclaw -v $ATP_REPO_PATH:/home/node/.openclaw/workspace:ro -v $OPENCLAW_HOME_DIR/agents:/home/node/openclaw/agents --name openclaw $OPENCLAW_IMAGE_FALLBACK"
