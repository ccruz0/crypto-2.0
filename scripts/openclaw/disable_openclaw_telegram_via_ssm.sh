...
cmd_id=$(aws ssm send-command \
  ...
  --timeout-seconds 300 \
  --parameters "commands=[\"export HOME=/root; git config --global --add safe.directory $REPO_ON_LAB; cd $REPO_ON_LAB && git pull origin main 2>/dev/null || true; sudo bash $REPO_ON_LAB/scripts/openclaw/disable_openclaw_telegram.sh\"]" \
