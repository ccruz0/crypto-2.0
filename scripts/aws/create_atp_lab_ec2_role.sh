#!/usr/bin/env bash
# LAB instance role — create atp-lab-ec2-role, attach least-privilege runtime policy,
# associate instance profile to LAB, set IMDS hop limit 2 so Docker can use the role.
#
# Does NOT edit env files and does NOT restart containers.
# Does NOT touch PROD i-087953603011543c5.
...
INLINE_POLICY_NAME="atp-lab-runtime"
INSTANCE_ID="i-0d82c172235770a0d"
EXISTING_ROLE="EC2_SSM_Role"
TRUST_FILE="$ROOT/ci-cd/iam/trust-atp-lab-ec2.json"
POLICY_FILE="$ROOT/ci-cd/iam/policy-atp-lab-runtime.json"
...
echo "=== LAB instance role: instance role for LAB $INSTANCE_ID ==="
...
echo "=== Current instance profile on LAB ==="
...
if aws iam get-role --role-name "$ROLE_NAME" >/dev/null 2>&1; then
  ...
  aws iam create-role \
    ...
    --description "ATP LAB EC2 instance role: SSM + Bedrock invoke (no static keys, no prod trading)"
...
echo "=== Associate profile to LAB (no instance reboot) ==="
...
if [[ "$VERIFY_OK" -eq 1 ]]; then
  ...
  echo "Role attached. Next: AWS_PROFILE=carlos-sso ./scripts/aws/switch_lab_to_instance_role.sh"
  echo "If Docker still cannot assume the role after stripping keys, hop limit is already 2."
else
