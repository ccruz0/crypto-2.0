...
SSM_BRIEF_TELEGRAM_CHAT_ID="/automated-trading-platform/prod/brief/telegram_chat_id"
SSM_BRIEF_TELEGRAM_BOT_TOKEN="/automated-trading-platform/prod/brief/telegram_bot_token"
SSM_TELEGRAM_WEBHOOK_SECRET="/automated-trading-platform/prod/brief/telegram_webhook_secret"
SSM_BRIEF_GRAPH_TENANT_ID="/automated-trading-platform/prod/brief/graph_tenant_id"
SSM_BRIEF_GRAPH_CLIENT_ID="/automated-trading-platform/prod/brief/graph_client_id"
SSM_BRIEF_GRAPH_CLIENT_SECRET="/automated-trading-platform/prod/brief/graph_client_secret"
...
PRESERVE_JARVIS_PROMOTE_PR_ENABLED="$(read_runtime_env_flag JARVIS_PROMOTE_PR_ENABLED)"
PRESERVE_JARVIS_LAB_TRIAL_ENABLED="$(read_runtime_env_flag JARVIS_LAB_TRIAL_ENABLED)"
...
PRESERVE_BRIEF_TELEGRAM_CHAT_ID="$(read_runtime_env_flag BRIEF_TELEGRAM_CHAT_ID)"
PRESERVE_BRIEF_TELEGRAM_BOT_TOKEN="$(read_runtime_env_flag BRIEF_TELEGRAM_BOT_TOKEN)"
PRESERVE_TELEGRAM_WEBHOOK_SECRET="$(read_runtime_env_flag TELEGRAM_WEBHOOK_SECRET)"
PRESERVE_BRIEF_GRAPH_TENANT_ID="$(read_runtime_env_flag BRIEF_GRAPH_TENANT_ID)"
PRESERVE_BRIEF_GRAPH_CLIENT_ID="$(read_runtime_env_flag BRIEF_GRAPH_CLIENT_ID)"
PRESERVE_BRIEF_GRAPH_CLIENT_SECRET="$(read_runtime_env_flag BRIEF_GRAPH_CLIENT_SECRET)"
...
# Phase C: promote flag stays off unless already enabled on host (never auto-on).
JARVIS_PROMOTE_PR_ENABLED="${PRESERVE_JARVIS_PROMOTE_PR_ENABLED:-false}"
JARVIS_LAB_TRIAL_ENABLED="${PRESERVE_JARVIS_LAB_TRIAL_ENABLED:-true}"
for _jarvis_key in \
...
printf "JARVIS_PROMOTE_PR_ENABLED=%s\n" "$JARVIS_PROMOTE_PR_ENABLED" >> "$RUNTIME_ENV"
printf "JARVIS_LAB_TRIAL_ENABLED=%s\n" "$JARVIS_LAB_TRIAL_ENABLED" >> "$RUNTIME_ENV"
JARVIS_4B_SOURCE=default
[[ -n "$PRESERVE_JARVIS_4B_PROPOSALS_ENABLED" ]] && JARVIS_4B_SOURCE=preserved
JARVIS_PHASE5_SOURCE=default
if [[ -n "$PRESERVE_JARVIS_PATCH_APPLY_ENABLED" || -n "$PRESERVE_JARVIS_PR_CREATION_ENABLED" || -n "$PRESERVE_JARVIS_GITHUB_WRITE_ENABLED" || -n "$PRESERVE_JARVIS_REQUIRE_DOUBLE_APPROVAL" || -n "$PRESERVE_JARVIS_PROMOTE_PR_ENABLED" || -n "$PRESERVE_JARVIS_LAB_TRIAL_ENABLED" ]]; then
...
BRIEF_ICS_URLS_VAL=""
BRIEF_TELEGRAM_CHAT_ID_VAL=""
BRIEF_TELEGRAM_BOT_TOKEN_VAL=""
TELEGRAM_WEBHOOK_SECRET_VAL=""
BRIEF_GRAPH_TENANT_ID_VAL=""
BRIEF_GRAPH_CLIENT_ID_VAL=""
BRIEF_GRAPH_CLIENT_SECRET_VAL=""
TELEGRAM_API_ID_VAL=""
TELEGRAM_API_HASH_VAL=""
TELEGRAM_SESSION_PATH_VAL=""
BRIEF_SOURCE="none"
BRIEF_GRAPH_SOURCE="none"
...
if command -v aws >/dev/null 2>&1 && aws sts get-caller-identity >/dev/null 2>&1; then
  ...
  BRIEF_TELEGRAM_CHAT_ID_VAL="$(fetch_ssm "$SSM_BRIEF_TELEGRAM_CHAT_ID" || true)"
  BRIEF_TELEGRAM_BOT_TOKEN_VAL="$(fetch_ssm "$SSM_BRIEF_TELEGRAM_BOT_TOKEN" || true)"
  TELEGRAM_WEBHOOK_SECRET_VAL="$(fetch_ssm "$SSM_TELEGRAM_WEBHOOK_SECRET" || true)"
  _ssm_graph_tenant="$(fetch_ssm "$SSM_BRIEF_GRAPH_TENANT_ID" || true)"
  _ssm_graph_client="$(fetch_ssm "$SSM_BRIEF_GRAPH_CLIENT_ID" || true)"
  _ssm_graph_secret="$(fetch_ssm "$SSM_BRIEF_GRAPH_CLIENT_SECRET" || true)"
  ...
  if [[ -n "$_ssm_graph_tenant" && -n "$_ssm_graph_client" && -n "$_ssm_graph_secret" ]]; then
    BRIEF_GRAPH_TENANT_ID_VAL="$_ssm_graph_tenant"
    BRIEF_GRAPH_CLIENT_ID_VAL="$_ssm_graph_client"
    BRIEF_GRAPH_CLIENT_SECRET_VAL="$_ssm_graph_secret"
    BRIEF_GRAPH_SOURCE="ssm"
  fi
...
if [[ -z "$BRIEF_TELEGRAM_CHAT_ID_VAL" && -n "$PRESERVE_BRIEF_TELEGRAM_CHAT_ID" ]]; then
  BRIEF_TELEGRAM_CHAT_ID_VAL="$PRESERVE_BRIEF_TELEGRAM_CHAT_ID"
fi
if [[ -z "$BRIEF_GRAPH_TENANT_ID_VAL" || -z "$BRIEF_GRAPH_CLIENT_ID_VAL" || -z "$BRIEF_GRAPH_CLIENT_SECRET_VAL" ]]; then
  BRIEF_GRAPH_TENANT_ID_VAL=""
  BRIEF_GRAPH_CLIENT_ID_VAL=""
  BRIEF_GRAPH_CLIENT_SECRET_VAL=""
  BRIEF_GRAPH_SOURCE="none"
  if [[ -n "$PRESERVE_BRIEF_GRAPH_TENANT_ID" && -n "$PRESERVE_BRIEF_GRAPH_CLIENT_ID" && -n "$PRESERVE_BRIEF_GRAPH_CLIENT_SECRET" ]]; then
    BRIEF_GRAPH_TENANT_ID_VAL="$PRESERVE_BRIEF_GRAPH_TENANT_ID"
    BRIEF_GRAPH_CLIENT_ID_VAL="$PRESERVE_BRIEF_GRAPH_CLIENT_ID"
    BRIEF_GRAPH_CLIENT_SECRET_VAL="$PRESERVE_BRIEF_GRAPH_CLIENT_SECRET"
    BRIEF_GRAPH_SOURCE="preserved"
  fi
fi
...
if [[ -f "$ROOT_DIR/.env.aws" ]]; then
  ...
  if [[ -z "$BRIEF_TELEGRAM_CHAT_ID_VAL" ]]; then
    _v="$(_read_env_aws_key BRIEF_TELEGRAM_CHAT_ID)"
    [[ -n "$_v" ]] && BRIEF_TELEGRAM_CHAT_ID_VAL="$_v"
  fi
  if [[ -z "$BRIEF_GRAPH_TENANT_ID_VAL" || -z "$BRIEF_GRAPH_CLIENT_ID_VAL" || -z "$BRIEF_GRAPH_CLIENT_SECRET_VAL" ]]; then
    _env_g_tenant="$(_read_env_aws_key BRIEF_GRAPH_TENANT_ID)"
    _env_g_client="$(_read_env_aws_key BRIEF_GRAPH_CLIENT_ID)"
    _env_g_secret="$(_read_env_aws_key BRIEF_GRAPH_CLIENT_SECRET)"
    if [[ -n "$_env_g_tenant" && -n "$_env_g_client" && -n "$_env_g_secret" ]]; then
      BRIEF_GRAPH_TENANT_ID_VAL="$_env_g_tenant"
      BRIEF_GRAPH_CLIENT_ID_VAL="$_env_g_client"
      BRIEF_GRAPH_CLIENT_SECRET_VAL="$_env_g_secret"
      BRIEF_GRAPH_SOURCE="env.aws"
    else
      BRIEF_GRAPH_TENANT_ID_VAL=""
      BRIEF_GRAPH_CLIENT_ID_VAL=""
      BRIEF_GRAPH_CLIENT_SECRET_VAL=""
...
for _brief_key in \
  ...
  BRIEF_ICS_URLS \
  BRIEF_TELEGRAM_CHAT_ID \
  BRIEF_GRAPH_TENANT_ID \
  BRIEF_GRAPH_CLIENT_ID \
  BRIEF_GRAPH_CLIENT_SECRET \
  TELEGRAM_API_ID \
  TELEGRAM_API_HASH \
...
[[ -n "$BRIEF_TELEGRAM_CHAT_ID_VAL" ]] && printf "BRIEF_TELEGRAM_CHAT_ID=%s\n" "$BRIEF_TELEGRAM_CHAT_ID_VAL" >> "$RUNTIME_ENV"
[[ -z "$BRIEF_TELEGRAM_BOT_TOKEN_VAL" && -n "$PRESERVE_BRIEF_TELEGRAM_BOT_TOKEN" ]] && BRIEF_TELEGRAM_BOT_TOKEN_VAL="$PRESERVE_BRIEF_TELEGRAM_BOT_TOKEN"
[[ -n "$BRIEF_TELEGRAM_BOT_TOKEN_VAL" ]] && printf "BRIEF_TELEGRAM_BOT_TOKEN=%s\n" "$BRIEF_TELEGRAM_BOT_TOKEN_VAL" >> "$RUNTIME_ENV"
[[ -z "$TELEGRAM_WEBHOOK_SECRET_VAL" && -n "$PRESERVE_TELEGRAM_WEBHOOK_SECRET" ]] && TELEGRAM_WEBHOOK_SECRET_VAL="$PRESERVE_TELEGRAM_WEBHOOK_SECRET"
[[ -n "$TELEGRAM_WEBHOOK_SECRET_VAL" ]] && printf "TELEGRAM_WEBHOOK_SECRET=%s\n" "$TELEGRAM_WEBHOOK_SECRET_VAL" >> "$RUNTIME_ENV"
echo "Present: BRIEF_TELEGRAM_BOT_TOKEN=$([[ -n "$BRIEF_TELEGRAM_BOT_TOKEN_VAL" ]] && echo YES || echo NO) TELEGRAM_WEBHOOK_SECRET=$([[ -n "$TELEGRAM_WEBHOOK_SECRET_VAL" ]] && echo YES || echo NO)"
if [[ -n "$BRIEF_GRAPH_TENANT_ID_VAL" && -n "$BRIEF_GRAPH_CLIENT_ID_VAL" && -n "$BRIEF_GRAPH_CLIENT_SECRET_VAL" ]]; then
  printf "BRIEF_GRAPH_TENANT_ID=%s\n" "$BRIEF_GRAPH_TENANT_ID_VAL" >> "$RUNTIME_ENV"
  printf "BRIEF_GRAPH_CLIENT_ID=%s\n" "$BRIEF_GRAPH_CLIENT_ID_VAL" >> "$RUNTIME_ENV"
  printf "BRIEF_GRAPH_CLIENT_SECRET=%s\n" "$BRIEF_GRAPH_CLIENT_SECRET_VAL" >> "$RUNTIME_ENV"
fi
