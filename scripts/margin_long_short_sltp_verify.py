...
def create_sltp(symbol: str, entry_side: str) -> Tuple[bool, Optional[str], Optional[str], Optional[str], Optional[str]]:
    """Create SL/TP via the unauthenticated test helper (supports long+short).

    Does not call ``/orders/create-sl-tp-for-last-order`` — that route is
    path-guard protected and (on main) still long-only (BUY).
    """
    side_q = urllib.parse.quote(entry_side.upper())
    sym_q = urllib.parse.quote(symbol.upper())
    url = f"{BASE}/test/create-sl-tp-last/{sym_q}?entry_side={side_q}"
