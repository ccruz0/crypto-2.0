#!/usr/bin/env python3
"""
Servidor con timestamp preciso para Crypto.com API
"""
...
def get_precise_timestamp():
    """Obtener timestamp preciso para Crypto.com API"""
    try:
        # Intentar obtener tiempo de servidor confiable
        response = requests.get('http://worldtimeapi.org/api/timezone/UTC', timeout=3)
        if response.status_code == 200:
            data = response.json()
            utc_time = data['datetime']
            dt = datetime.fromisoformat(utc_time.replace('Z', '+00:00'))
            timestamp = int(dt.timestamp() * 1000)
            logger.info(f"🕐 Timestamp desde servidor UTC: {timestamp} ({dt})")
            return timestamp
    except:
        pass
    
    # Fallback: corregir timestamp local
    current_time = time.time()
    if datetime.fromtimestamp(current_time).year == 2025:
        # Restar exactamente 1 año (365 días)
        corrected_time = current_time - (365 * 24 * 60 * 60)
        timestamp = int(corrected_time * 1000)
        logger.info(f"🕐 Timestamp corregido localmente: {timestamp} ({datetime.fromtimestamp(corrected_time)})")
        return timestamp
    else:
        timestamp = int(current_time * 1000)
        logger.info(f"🕐 Timestamp del sistema: {timestamp} ({datetime.fromtimestamp(current_time)})")
        return timestamp
...
def make_authenticated_request(method, params=None):
    """Realizar petición autenticada con timestamp preciso"""
    try:
        nonce = get_precise_timestamp()
...
def health():
    return jsonify({
        ...
        "message": "Conectándose a TU cartera real con timestamp preciso"
    })

if __name__ == '__main__':
    print("🚀 Iniciando servidor con timestamp preciso...")
    print("📡 Endpoint: http://localhost:8007/api")
    print("✅ Credenciales reales configuradas")
    print("🕐 Timestamp preciso para Crypto.com API")
