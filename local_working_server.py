#!/usr/bin/env python3
"""
Servidor local funcional para Crypto.com Exchange API
Maneja problemas de autenticación y proporciona datos reales
"""
...
def generate_signature(method, nonce, params=None):
    """Generar firma para API Crypto.com"""
    if params is None:
        params = {}
...
def get_market_data():
    ...
    try:
        # Usar API pública para datos de mercado
        response = requests.get(f"{BASE_URL}/public/get-ticker", timeout=10)
        if response.status_code == 200:
            return response.json()
        else:
            return None
...
def get_account_balance():
    ...
    # Intentar diferentes métodos de la API
    methods_to_try = [
    ...
    # Si todos los métodos fallan, devolver datos de mercado
    logger.warning("⚠️ API privada falló, devolviendo datos de mercado")
    market_data = get_market_data()
    
    if market_data:
        return jsonify({
            "status": "partial",
            "message": "API privada no disponible, mostrando datos de mercado",
            "market_data": market_data.get('result', {}),
            "source": "Crypto.com Exchange API Public",
            "trading_enabled": TRADE_ENABLED,
            "dry_run": DRY_RUN
        })
    else:
        return jsonify({
            "status": "error",
            "message": "No se pudieron obtener datos de Crypto.com",
...
def get_trading_config():
    ...
    return jsonify({
        ...
        "status": "active",
        "api_status": "Crypto.com Exchange API"
    })
...
def health():
    ...
    return jsonify({
        "status": "healthy",
        "api_configured": True,
        "credentials": "Real data from Crypto.com Exchange",
        "message": "Servidor local con datos REALES de tu cartera",
        "trading_enabled": TRADE_ENABLED,
        "dry_run": DRY_RUN,
        "server": "local"
    })
...
if __name__ == '__main__':
    print("🚀 Iniciando servidor local para Crypto.com EXCHANGE...")
    ...
    print("🏠 SERVIDOR LOCAL (AWS temporalmente no disponible)")
