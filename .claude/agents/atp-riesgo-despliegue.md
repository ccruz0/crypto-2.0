...
- **La retención de este repo tiene DOS agujeros conocidos, no uno.** Compruébalos ambos antes de decir que un cambio está retenido:
  1. **El push directo a `main` salta las reglas.** Desde una cuenta con permiso de bypass, `git push origin main` entra igual y GitHub lo reporta como `Bypassed rule violations`: se saltan tanto *"Changes must be made through a pull request"* como el check obligatorio **`path-guard`**, que **no llega a ejecutarse**. Confirmado el 21-ago-2026 (commit `e9162cc5`). Si alguien dice *"lo subo directo, es solo configuración"*, el guardián de rutas no corre.
